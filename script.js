...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initWeatherEffect();
  initHeaderScroll();
  initHeroHeadlineSpotlight();
  initCleanScrollLinks();
});
...
/**
 * Interactive Spotlight effect on the Hero headline
 */
function initHeroHeadlineSpotlight() {
  ...
  const handleMouseMove = (e) => {
    ...
    headline.classList.add("spotlight-active");
  };

  const handleMouseLeave = () => {
    headline.classList.remove("spotlight-active");
  };
...
/**
 * Smooth scroll to sections without updating hash in URL, and clean hashes on load
 */
function initCleanScrollLinks() {
  // 1. Handle in-page anchor clicks
  const links = document.querySelectorAll('a[href^="#"], a[href*="/#"], a[href*="index.html#"]');
  ...
  links.forEach(link => {
    link.addEventListener("click", (e) => {
      ...
      if (href.includes("index.html#") && isIndexPage) {
        href = href.split("index.html")[1]; // Get the #section part
      } else if (href.includes("/#") && isIndexPage) {
        href = "#" + href.split("/#")[1]; // Get the #section part
      }
  ...
  // 2. Handle scroll target on initial load if hash exists
  if (window.location.hash) {
    const targetId = window.location.hash;
    const targetEl = document.querySelector(targetId);
    if (targetEl) {
      // Temporarily scroll to top first to prevent default jump
      window.scrollTo(0, 0);
      
      // After a tick, smoothly scroll to offset position and clear hash
      setTimeout(() => {
        const header = document.querySelector("header");
        const headerHeight = header ? header.offsetHeight : 0;
        const targetPosition = targetEl.getBoundingClientRect().top + window.scrollY - headerHeight;
